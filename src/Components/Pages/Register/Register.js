...
export default function Register() {
  ...
  const validate = async () => {
    ...
    if (!username.trim()) e.username='Username is required.';
      ...
      const q = query(collection(db,'user_form'), where('username','==',username.trim()));
    ...
    if (!email.trim()) e.email='Email is required.';
      ...
      const q = query(collection(db,'user_form'), where('email','==',email.trim()));
      if (!(await getDocs(q)).empty)
        e.email='This email has been taken. Please use other email.';
    }
    if (password.length<8) e.password='Password must be at least 8 characters.';
    if (password!==cpassword) e.cpassword='Passwords do not match!';
    if (!terms) e.terms='Please tick this before proceeding.';
    return e;
  };
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      navigate('/login',{ state:{ welcome_msg:'Thanks for signing up. You may now sign in.' } });
    } catch(err) {
    ...
    <main>
      <div className="row">
        ...
        <div className="column middle">
          ...
          <p style={{textAlign:'center'}}>Create a free account to use our service</p>
                ...
                <tr className="form-row">
                  <td colSpan="2" className="form-group w-100">
                    <div className="checkbox-container">
                      ...
                      />
                      <label>
                        I agree to the <a href="#">Terms</a> and acknowledge the <a href="#">
